...
void EQStream::SendPacket(uint16 opcode, EQApplicationPacket *p)
{
	...
	if (p->size>(MaxLen-8)) { // proto-op(2), seq(2), app-op(2) ... data ... crc(2)
		Log(Logs::General, Logs::Netcode, _L "Making oversized packet, len [{0}]" __L, p->Size());

		auto tmpbuff = new unsigned char[p->size + 3];
		length = p->serialize(opcode, tmpbuff);
		if (length != p->Size())
			Log(Logs::General, Logs::Netcode, _L "Packet adjustment, len [{0}] to [{1}]" __L, p->Size(), length);
		...
		*(uint32 *)(out->pBuffer + 2) = htonl(length);
		...
		Log(Logs::General, Logs::Netcode, _L "First fragment: used [{0}]/[{1}]. Payload size [{2}] in the packet" __L, used, length, p->size);
		SequencedPush(out);
		...
		while (used<length) {
			...
			SequencedPush(out);
			used += chunksize;
			Log(Logs::General, Logs::Netcode, _L "Subsequent fragment: len [{0}], used [{1}]/[{2}]." __L, chunksize, used, length);
		}
		delete p;
		delete[] tmpbuff;
	} else {
...
void EQStream::SequencedPush(EQProtocolPacket * p)
{
		MOutboundQueue.lock();
		if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
			Log(Logs::General, Logs::Netcode, _L "Pre-Push Invalid Sequenced queue: BS [{0}] + SQ [{1}] != NOS [{2}]" __L, SequencedBase, SequencedQueue.size(), NextOutSeq);
		}
		if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
			Log(Logs::General, Logs::Netcode, _L "Pre-Push Next Send Sequence is beyond the end of the queue NSS [{0}] > SQ [{1}]" __L, NextSequencedSend, SequencedQueue.size());
		}

		Log(Logs::General, Logs::Netcode, _L "Pushing sequenced packet [{0}] of length [{1}]. Base Seq is [{2}]." __L, NextOutSeq, p->size, SequencedBase);
		*(uint16*)(p->pBuffer) = htons(NextOutSeq);
		SequencedQueue.push_back(p);
		NextOutSeq++;

		if (uint16(SequencedBase + SequencedQueue.size()) != NextOutSeq) {
			Log(Logs::General, Logs::Netcode, _L "Push Invalid Sequenced queue: BS [{0}] + SQ [{1}] != NOS [{2}]" __L, SequencedBase, SequencedQueue.size(), NextOutSeq);
		}
		if (NextSequencedSend > (SequencedBase + SequencedQueue.size())) {
			Log(Logs::General, Logs::Netcode, _L "Push Next Send Sequence is beyond the end of the queue NSS [{0}] > SQ [{1}]" __L, NextSequencedSend, SequencedQueue.size());
		}
		MOutboundQueue.unlock();
}
...
{
	MOutboundQueue.lock();
	Log(Logs::General, Logs::Netcode, _L "Pushing non-sequenced packet of length [{0}]" __L, p->size);
...
uint16 Seq=htons(seq);
	Log(Logs::Detail, Logs::Netcode, _L "Sending ack with sequence [{0}]" __L, seq);
	SetLastAckSent(seq);
...
{
	Log(Logs::Detail, Logs::Netcode, _L "Sending out of order ack with sequence [{0}]" __L, seq);
	uint16 Seq=htons(seq);
